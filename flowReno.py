...
class FlowReno():
    ...
    def updateW(self):
        if self.fast_recovery:
            ...
            if self.windowSize <= self.sst:     # Slow start phase
                ...
                self.windowSize = float(self.windowSize) + 1.0/float(math.floor(self.windowSize))
    ...
    def getACK(self, packetID, ackTime):
        ...
        if packetID > self.last_unackd:
            ...
            # If we're in the fast recovery phase and we received a packet request
            #   for a packet in this window, then send it
            if self.fast_recovery and self.last_unackd <= self.fast_recovery_pkts:
                pkt = DataPacket(self.last_unackd, self.source, self.dest, self.ID, constants.system_EQ.currentTime)
            ...
            lengthPktsToSend = math.ceil(self.getWindowSize()) - len(self.unackPackets)
            ...
            if self.windowSize > self.sst and not self.fast_recovery and self.dupAckCtr == 3:
                self.sst = max(float(self.windowSize)/2.0, 1.0)
                self.windowSize = math.ceil(self.sst)
                self.logWindowSize()
